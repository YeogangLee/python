...
flag_ing = [True]
omok_size = [19]
a = Audio(sound_file_name='baduk.mp3', pitch=1, loop=False, autoplay=False)
...
btn_v = Button(text='**승리', origin=(0,0), scale=(0.11, 0.07, 1), background=True, enabled=False)
btn_v.z = -25
...
def myclick(i,j):
    ...
    if flag_wb[0] :
        ...
        arr2D[i][j] = 2
        stone = 2
        Audio(sound_file_name='baduk.mp3', pitch=1, loop=False, autoplay=False)
...
for i in range(omok_size[0]) :
    ...
    for j in range(omok_size[0]) :
        ...
        sp = Entity(model='sphere', color=color.black, scale=(0.8,0.3,0.8))
        # sp.x = j - 9
        # sp.y = - i + 9
        sp.z = 9 - i
        sp.x = -9 + j
