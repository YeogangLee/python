...
mail_info = ["수신자 이메일", "이메일 제목", "이메일 본문"]

@app.route('/', methods=['POST','GET'])
@app.route('/send_email', methods=['POST','GET'])
def show_send_mail():
    # mail_info.append(request.form['email']) 
    # mail_info.append(request.form['title']) 
    # mail_info.append(request.form['text'])   
    
    mail_info[0] = request.form['email']
    mail_info[1] = request.form['title']
    mail_info[2] = request.form['text']
    
    print(mail_info[0], mail_info[1], mail_info[2])
